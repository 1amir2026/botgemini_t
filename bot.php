...
// --------------------------------------------------------
// لیست پاسخ‌های شانسی و طعنه‌آمیز برای وقتی اسم «پیکسیا» شنیده می‌شود
// --------------------------------------------------------
function bot_pixia_disses() {
    return [
        "پیکسیا کیه؟ نشناختم 🤔",
        "پیکسیا؟ این اسمو اولین باره می‌شنوم، حتما کسی نیست که ارزش شناختن داشته باشه 😅",
        "پیکسیا رو نمی‌شناسم و فکر نکنم چیز خاصی از دست داده باشم 🙄",
        "کی گفته پیکسیا؟ من که اصلا همچین چیزی تو لیست آدمای مهم ندارم 😂",
        "پیکسیا؟ بی‌خیال، حتی گوگل هم بهتر از این می‌شناسه که من نمی‌شناسمش 🤷‍♂️",
        "اسم پیکسیا رو هیچ‌جا نشنیدم، احتمالا چیز مهمی نیست 😏",
        "پیکسیا کیه اصلا؟ یه چیزی تو مایه‌های گمنامی به نظر میاد 😬",
        "نه بابا پیکسیا رو نمی‌شناسم، شاید باید بیشتر تلاش کنه دیده بشه 😅",
    ];
}

// --------------------------------------------------------
// تشخیص اینکه آیا ربات با اسم Bloxy/بلاکسی/بلوکسی صدا زده شده یا نه
// --------------------------------------------------------
function bot_is_called_by_name($text) {
    if (trim((string)$text) === '') return false;
    $lower = mb_strtolower($text);
    return (preg_match('/\bbloxy\b/iu', $text) === 1)
...
// فقط زمانی فعال می‌شود که ربات با اسم Bloxy/بلاکسی صدا زده شده باشد.
...
function bot_handle_special_triggers($text, $chat_id, $message_id) {
    if (trim((string)$text) === '') return false;
    if (!bot_is_called_by_name($text)) return false; // فقط وقتی صدا زده شده

    $lower = mb_strtolower($text);

    // ایستر اگ: کلمه «پیکسیا» در پیام باشد -> یکی از جواب‌های طعنه‌آمیز به‌صورت شانسی
    if (mb_strpos($text, 'پیکسیا') !== false || mb_strpos($lower, 'pixia') !== false) {
        $disses = bot_pixia_disses();
        bot('sendMessage', [
            'chat_id' => $chat_id,
            'reply_to_message_id' => $message_id,
            'text' => $disses[array_rand($disses)]
        ]);
        return true;
    }
